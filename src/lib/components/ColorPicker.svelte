<script>
     ...
     * @param {number} blue
     */
    export function set(red, green, blue) {
        r = red;
        g = green;
        b = blue;
    }
...
<article>
    <label for="brightness"
        ...
        <input
            ...
            value={brightness}
            on:input={(ev) => {
                console.debug("on:input brightness", parseInt(ev.currentTarget.value), r, g, b);
                setColorForBrightness(parseInt(ev.currentTarget.value));
            }}
        />
    </label>
